defmodule Phpbb2Crawler do
  ...
  def new(base_url, opts \\ []) do
    %Phpbb2Crawler{base_url: base_url, cookie: nil}
  end
  ...
  def login(base_url, username, password, opts \\ []) do
    ...
    case Finch.build(:post, login_url, headers, form_data) |> Finch.request(Phpbb2Crawler.Finch) do
      {:ok, %Finch.Response{status: status, headers: resp_headers}} when status in [200, 302] ->
        ...
        if cookie do
          ...
          {:ok, %Phpbb2Crawler{base_url: base_url, cookie: cookie}}
        else
  ...
  """
  def crawl_all(%Phpbb2Crawler{} = crawler, concurrency \\ @concurrency, timeout \\ @timeout) do
  ...
  def fetch_forums(%Phpbb2Crawler{} = crawler) do
  ...
  defp get_request(%Phpbb2Crawler{cookie: cookie}, url, receive_timeout \\ 15_000) do
    headers = if cookie, do: [{"cookie", cookie}], else: []

    case Finch.build(:get, url, headers) |> Finch.request(Phpbb2Crawler.Finch, receive_timeout: receive_timeout) do
